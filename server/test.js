...
test('empty setup, availability, booking and room service use real persisted data',async t=>{
 ...
 assert.equal(bookingRules.status,200);assert.equal(bookingRules.data.max_stay_nights,14);assert.equal(bookingRules.data.require_phone,false);
 ...
 const stateWithDepartments=await call('GET','/api/admin/state');
 const housekeepingDepartment=stateWithDepartments.data.departments.find(x=>x.name==='Housekeeping');
 const routed=await call('PUT',`/api/admin/requests/${req.data.request.id}`,{priority:'urgent',department_id:housekeepingDepartment.id,assigned_to:'Floor team',status:'in_progress'});
 assert.equal(routed.status,200);assert.equal(routed.data.request.priority,'urgent');assert.equal(routed.data.request.status,'in_progress');
 ...
 const preference=await call('POST','/api/admin/preferences',{guest_id:guestId,category:'Arrival',preference:'Quiet room away from lifts',sensitivity:'private'});
 assert.equal(preference.status,201);
 const housekeeping=await call('POST','/api/admin/housekeeping',{room_id:room1.id,reservation_id:view.data.reservation.id,task_type:'arrival',priority:'high',scheduled_for:'2027-02-01',assigned_to:'Amina'});
 assert.equal(housekeeping.status,201);
 const maintenance=await call('POST','/api/admin/maintenance',{room_id:room1.id,title:'Inspect climate control',priority:'normal',status:'open',out_of_order:true});
 assert.equal(maintenance.status,201);
 const reducedInventory=await call('GET','/api/public/availability?checkIn=2027-03-01&checkOut=2027-03-02&adults=2&children=0');
 assert.equal(reducedInventory.data.types.find(x=>x.id===type.id).available_count,1);
 assert.equal((await call('PUT',`/api/admin/maintenance/${maintenance.data.id}`,{status:'resolved'})).status,200);
 const restoredInventory=await call('GET','/api/public/availability?checkIn=2027-03-01&checkOut=2027-03-02&adults=2&children=0');
 assert.equal(restoredInventory.data.types.find(x=>x.id===type.id).available_count,2);
 const section=await call('POST','/api/admin/sections',{section_key:'signature-service',nav_label:'Service',heading:'Service, remembered',body:'A configurable editorial section.',layout:'split',enabled:true});
 assert.equal(section.status,201);assert.ok((await call('GET','/api/public/site')).data.sections.some(x=>x.section_key==='signature-service'));
 assert.equal((await call('PUT','/api/admin/features/guest_services',{enabled:false})).status,200);
 assert.equal((await call('POST',`/api/public/reservations/${first.data.token}/requests`,{title:'Blocked'})).status,409);
 assert.equal((await call('PUT','/api/admin/features/guest_services',{enabled:true})).status,200);
 const operationsState=await call('GET','/api/admin/state');
 assert.equal(operationsState.data.overview.rooms_to_service,1);assert.equal(operationsState.data.overview.open_maintenance,0);assert.equal(operationsState.data.preferences.length,1);
