...
test('empty setup, availability, booking and room service use real persisted data',async t=>{
 ...
 const maintenance=await call('POST','/api/admin/maintenance',{room_id:room1.id,title:'Inspect climate control',priority:'normal',status:'open',out_of_order:false});
 ...
 assert.equal(operationsState.data.overview.rooms_to_service,1);assert.equal(operationsState.data.overview.open_maintenance,1);assert.equal(operationsState.data.preferences.length,1);
